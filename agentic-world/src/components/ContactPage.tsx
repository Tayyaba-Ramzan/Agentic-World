...
const ContactPage = () => {
  ...
  return (
    <section id="contact" className="relative w-full min-h-screen flex flex-col items-center justify-center bg-black text-white px-6 py-16 overflow-hidden">
      {/* Background Glow */}
      <div className="absolute inset-0 bg-gradient-to-br from-purple-800 via-black to-blue-800 opacity-40 blur-2xl"></div>
      ...
      <motion.h1
        data-aos="fade-down"
        className="text-5xl font-bold bg-gradient-to-r from-purple-400 to-pink-600 bg-clip-text text-transparent animate-pulse text-center w-full max-w-3xl mt-12"
      >
        Get in Touch
      </motion.h1>

      <p className="text-lg text-gray-400 mt-4 text-center max-w-2xl" data-aos="fade-up">
      Need help? Let’s connect and discuss your project.
      </p>
      ...
      <motion.div className="relative z-10 flex flex-col-reverse md:flex-row items-center gap-12 w-full max-w-6xl pt-12">
        <motion.div
          data-aos="fade-up"
          className="w-full md:w-1/2 bg-white/10 p-8 rounded-xl shadow-lg backdrop-blur-md border border-white/20"
        >
            ...
            {["name", "email", "subject"].map((field) => (
              <div key={field}>
                ...
                {errors[field as keyof FormData] && (
                  <p className="text-red-400 text-sm">{errors[field as keyof FormData]?.message}</p>
                )}
              </div>
            ))}
